...
@app.route("/registercustmnmg", methods = ['GET', 'POST'])#
@is_logged_in
...
def editprofilehos(docid):
    session['url'] = 'editprofilehos/' + docid
...
@app.route("/addappointmenthosp", methods = ['GET', 'POST'])#
@is_logged_in
                ...
                if DocName == "":
                    ...
                    if datetime == "":
                        ...
                        if duplicate == []:
                            ...
                            return redirect(url_for('addappointmenthosp'))
    if PatUsername == "":
        return render_template('addappointmenthosp.html', patuser = PatUsername, pat = db.session.query(CustomerDet).order_by(CustomerDet.custid.asc()).all(), prob = problem, specs = db.session.query(StaffDet).filter(StaffDet.hospitalid == username).order_by(StaffDet.docid.asc()).all(), spec = specialisation, doc = DocName)
    else:
        if specialisation == "":
            return render_template('addappointmenthosp.html', patuser = PatUsername, pat = db.session.query(CustomerDet).order_by(CustomerDet.custid.asc()).all(), prob = problem, specs = db.session.query(StaffDet).filter(StaffDet.hospitalid == username).order_by(StaffDet.docid.asc()).all(), spec = specialisation, doc = DocName)
        else:
            if DocName == "":
                return render_template('addappointmenthosp.html', patuser = PatUsername, pat = db.session.query(CustomerDet).order_by(CustomerDet.custid.asc()).all(), prob = problem, specs = db.session.query(StaffDet).filter(StaffDet.hospitalid == username).order_by(StaffDet.docid.asc()).all(), spec = specialisation, hosps = db.session.query(StaffDet).filter(StaffDet.spec == specialisation).order_by(StaffDet.docid.asc()).all(), doc = DocName)
            else:
                return render_template('addappointmenthosp.html', patuser = PatUsername, pat = db.session.query(CustomerDet).order_by(CustomerDet.custid.asc()).all(), prob = problem, specs = db.session.query(StaffDet).filter(StaffDet.hospitalid == username).order_by(StaffDet.docid.asc()).all(), spec = specialisation, hosps = db.session.query(StaffDet).filter(StaffDet.spec == specialisation, StaffDet.hospitalid == username).order_by(StaffDet.docid.asc()).all(), doc = DocName)

@app.route("/aptaccept/<num>", methods = ['GET', 'POST'])#
@is_logged_in
def aptaccept(num):
...
@app.route("/aptdecline/<num>", methods = ['GET', 'POST'])#
@is_logged_in
def aptdecline(num):
...
@app.route("/addappointmentcust", methods = ['GET', 'POST'])#
@is_logged_in
def addappointmentcust():
    ...
    if specialisation == "":
        return render_template('addappointmentcust.html', prob = problem, specs = db.session.query(StaffDet).order_by(StaffDet.docid.asc()).all(), spec = specialisation, hosp = HospName)
    else:
        if HospName == "":
            return render_template('addappointmentcust.html', prob = problem, specs = db.session.query(StaffDet).order_by(StaffDet.docid.asc()).all(), spec = specialisation, hosps = db.session.query(StaffDet).filter(StaffDet.spec == specialisation).order_by(StaffDet.docid.asc()).all(), hosp = HospName)
        else:
            return render_template('addappointmentcust.html', prob = problem, specs = db.session.query(StaffDet).order_by(StaffDet.docid.asc()).all(), spec = specialisation, hosps = db.session.query(StaffDet).filter(StaffDet.spec == specialisation, StaffDet.hospitalid == HospName).order_by(StaffDet.docid.asc()).all(), hosp = HospName)

@app.route("/deleteappoint/<number>", methods = ['GET', 'POST'])#
@is_logged_in
...
@app.route("/addtablerow/<number>", methods = ['GET', 'POST'])
@is_admin
...
@app.route("/edittablerow/<number>", methods = ['GET', 'POST'])
@is_admin
...
@app.route("/editrow/<chumma>", methods = ['GET', 'POST'])
def editrow(chumma):
    number = session['number']
    if number == '1':
        ...
        return render_template('edittablerow.html', f = '', appoints = db.session.query(Appointments).filter(Appointments.id == chumma).first(), hosps = db.session.query(RegisterMnmg).all(), pats = db.session.query(CustomerDet).all(), specs = db.session.query(StaffDet).all(), doctors = db.session.query(StaffDet).filter(StaffDet.hospitalid == hospitall).all(), origstat = origstat)
    else:
        flash("No such table exists", "danger")
        return redirect(url_for('admindash'))
    return render_template('edittablerow.html')
